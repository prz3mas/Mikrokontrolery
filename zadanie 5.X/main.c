...
void suddenDeath(unsigned int time)
{
            
       unsigned int min1 = time;
       unsigned int min2 = time;
       unsigned int sec1 = 0 , sec2 = 0 ;
     ...
     while(1)
    {
       ...
       {
            char buffer[32];
            sprintf(buffer, " %02i:%02i    %02i:%02i       ", min1, sec1, min2, sec2);
       
                lcd_cmd(L_CLR);
                lcd_cmd(L_L1); //Ustawienie karetki w pierwszej linii
                lcd_str("Zaw 1    Zaw 2      "); //napis
                lcd_cmd(L_L2); //Przej??ie do drugiej linii
                lcd_str(buffer);
                delay(1000);
                sec1--;
                if((sec1 > 59)|(sec1 < 0)){sec1 = 59; min1--;}
                if((sec1 <= 0) & (min1 <= 0))
                {
       ...
       while(PORTBbits.RB3 != 0)
       {
            ...
            delay(1000);
            sec2--;
            if((sec2 > 59)|(sec2 < 0)){sec2 = 59; min2--;}
            if((sec2 == 0) & (min2 == 0))
            {
                while(PORTBbits.RB1 != 0)
                {
...
void main(void) {
    ...
    while(PORTBbits.RB3 != 0)
    {
        tmp=((unsigned int)adc(1) / 64);
        display = (unsigned char)(tmp);
        delay(200);
        
        if(display <= 3){time = 1;}
        else if((display > 3)  & (display <= 10)){time = 3;}
        else{time = 5;}       
        
        sprintf(buffer, "     %i min          ", time);
        
      
       lcd_cmd(L_CLR);
       lcd_cmd(L_L1); //Ustawienie karetki w pierwszej linii
       lcd_str("Ustaw czas:         "); //napis
       lcd_cmd(L_L2); //Przej??ie do drugiej linii
       lcd_str(buffer);
       delay(200);
       
       if(PORTBbits.RB3 == 0){suddenDeath(time);}
    }
